...
void spfa(tp s) {
  ...
  dist[s] = 0;
  vis[s] = 1;
  for (SLF.push_back(s); SLF.size();) {
    while (dist[SLF.front()] * SLF.size() > LLL) {
      SLF.push_back(SLF.front());
      SLF.pop_front();
    }
    tp loc = SLF.front();
    vis[loc] = 0;
    ...
    for (auto&& i : e[loc]) {
      if (dist[i.first] > dist[loc] + i.second) {
        ...
        if (!vis[i.first]) {
          vis[i.first] = 1;
          LLL += dist[i.first];
          if (SLF.size() && dist[i.first] > dist[loc]) {
            SLF.push_back(i.first);
          } else {
            SLF.push_front(i.first);
          }
        }
      }
    }
